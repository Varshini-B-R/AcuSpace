...
import { useAuth } from '../../lib/AuthContext'; // <-- NEW: Import Auth Context
...
export default function ToolsScreen() {
  ...
  // --- NEW: Grab the current user session ---
  ...
  const handlePlaceOrder = async () => {
    ...
    // Quick Auth Check
    if (!session?.user?.id) {
      setErrorMessage('⚠️ You must be logged in to place an order.');
      return;
    }
    ...
    try {
      // 1. Save to Database (Role Agnostic - just uses the active user ID)
      const { error: dbError } = await supabase.from('shop_orders').insert({
        user_id: session.user.id, // <-- Matches the new column name
